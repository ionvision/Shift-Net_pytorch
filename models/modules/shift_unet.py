...
# For bilinear attention
from models.bilinear_shift_net.innerBilinearShiftTriple import InnerBilinearShiftTriple
...
# if |num_downs| == 7, image of size 128x128 will become of size 1x1
# at the bottleneck
class BilinearUnetGeneratorShiftTripleUnetGeneratorShiftTriple(nn.Module):
    def __init__(self, input_nc, output_nc, num_downs, opt, innerCos_list, shift_list, mask_global, ngf=64,
                 norm_layer=nn.BatchNorm2d, use_dropout=False):
        super(BilinearUnetGeneratorShiftTripleUnetGeneratorShiftTriple, self).__init__()
        ...
        unet_shift_block = BilinearUnetSkipConnectionShiftBlock(ngf * 2, ngf * 4, opt, innerCos_list, shift_list,
                                                        mask_global, input_nc=None, \
                                                        submodule=unet_block,
                                                        norm_layer=norm_layer, ngf=ngf)  # passing in unet_shift_block
...
# Mention: the TripleBlock differs in `upconv` defination.
# 'cos' means that we add a `innerCos` layer in the block.
class BilinearUnetSkipConnectionShiftBlock(nn.Module):
    def __init__(self, outer_nc, inner_nc, opt, innerCos_list, shift_list, mask_global, input_nc, \
                 submodule=None, shift_layer=None, outermost=False, innermost=False, norm_layer=nn.BatchNorm2d,
                 use_dropout=False, ngf=None):
        super(BilinearUnetSkipConnectionShiftBlock, self).__init__()
        self.outermost = outermost
        if input_nc is None:
            input_nc = outer_nc
        ...
        # As the downconv layer is outer_nc in and inner_nc out.
        # So the shift define like this:
        dim = opt.fineSize / (inner_nc / ngf)
        shift = InnerBilinearShiftTriple(opt.fixed_mask, inner_nc, dim, shift_sz=opt.shift_sz, stride=opt.stride, triple_weight=opt.triple_weight) # fixed_mask, inner_nc, dim, shift_sz=1, stride=1, mask_thred=1, triple_weight=1
        ...
        # Add latent constraint
        # Then add the constraint to the constrain layer list!
        innerCosB = InnerCos(strength=opt.strength, skip=opt.skip)
        innerCosB.set_mask(mask_global, 3)  # Here we need to set mask for innerCos layer too.
        innerCos_list.append(innerCosB)

        # Add latent constraint
        # Then add the constraint to the constrain layer list!
        innerCosA = InnerCos(strength=opt.strength, skip=opt.skip)
        innerCosA.set_mask(mask_global, 3)  # Here we need to set mask for innerCos layer too.
        innerCos_list.append(innerCosA)
        ...
        if outermost:
            ...
            # NB: innerCos are placed before shift. So need to add the latent gredient to
            # to former part.
            up = [uprelu, innerCosB, shift, innerCosA, upconv, upnorm]
